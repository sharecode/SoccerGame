...
$title='ไม่พบสมาชิก';
$bill_rows=0;
if(isset($_GET['id'])&& isDecimal($_GET['id']))
{
	$tid=$_GET['id'];
	$get_user=$_con->prepare("select id,aliasname,round(score,2) as score from member_ where id=?;");
	$get_user->execute(array($tid));
	if($get_user->rowCount())
	{
		...
		$title='สถิติการทายผลของ '.$alias.' คะแนน '.$score;
		$get_bill=$_con->prepare("select * from bill where owner=? and bill_type=0 order by id desc limit $bill_show;");
		$get_bill->execute(array($uid));
	}
}
?>
<!DOCTYPE html>
...
<head> 
	...
	<meta name="keywords" content="<?php echo $alias; ?>"/>
		...
		for($bi=0;$bi<$get_bill->rowCount();$bi++)
		{
			...
			$match_data=$get_matchday->fetchAll();
				...
				echo '<tr>
							...
							<span class="team_choose">'.$side_choose.'</span> <span class="odds_choose">'.$at_odds.'</span><br>
